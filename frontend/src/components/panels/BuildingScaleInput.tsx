...
const READONLY_FIELDS: { key: keyof BuildingScaleFields; label: string; unit: string }[] = [
 ...
 * 지상층수/지하층수/건축면적/연면적 are all fully auto-applied from the
 * drawing (지상층수/지하층수 from FloorBar's 지상 marker, see
 * lib/floorOrder.ts; 건축면적/연면적 from every structure/floor add/edit/
 * remove, see useFloorPlanState's autoBuildingAreas) — shown read-only since
 * there's nothing to type. Only 대지면적 stays editable (auto-set once from
 * InitialSetupModal's 가로/세로, but adjustable afterward since it isn't
 * derivable from the drawing itself). */
  ...
  return (
    <div className="flex flex-col gap-2 rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm">
      ...
      <div className="flex items-center justify-between gap-2 text-xs text-gray-500">
        <span>지상/지하 층수 (자동 계산)</span>
        <span className="text-gray-700">
          지상 {value.buildingGroundFloorCount ?? 0}층 · 지하{" "}
          {value.buildingBasementFloorCount ?? 0}층
        </span>
      </div>
      {READONLY_FIELDS.map(({ key, label, unit }) => (
        <div key={key} className="flex items-center justify-between gap-2 text-xs text-gray-500">
          <span>{label} (자동 계산)</span>
          <span className="text-gray-700">
      ...
      <label className="flex items-center justify-between gap-2 text-xs text-gray-500">
        ...
        <span className="flex items-center gap-1">
          <input
            type="number"
            min={0}
            value={value.buildingSiteAreaSqm ?? ""}
            onChange={(e) => {
              const raw = e.target.value;
              onChange({
                buildingSiteAreaSqm: raw === "" ? undefined : Math.max(0, Number(raw)),
              });
            }}
